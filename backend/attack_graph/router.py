...
USER_ID = 1  # Replace with actual auth dependency when ready
...
def create_scenario(data: ScenarioCreate, db: Session = Depends(get_db)):
    ...
    scenario = Scenario(user_id=USER_ID, name=data.name, description=data.description)
    db.add(scenario)
    db.commit()
    db.refresh(scenario)
    return scenario
...
def add_asset(scenario_id: UUID, data: AssetCreate, db: Session = Depends(get_db)):
...
def get_assets(scenario_id: UUID, db: Session = Depends(get_db)):
...
def add_connection(scenario_id: UUID, data: ConnectionCreate, db: Session = Depends(get_db)):
    if data.port:
...
def get_connections(scenario_id: UUID, db: Session = Depends(get_db)):
...
def analyze_scenario(scenario_id: UUID, db: Session = Depends(get_db)):
    ...
    raw_paths = enumerate_attack_paths(assets, connections)
    ...
    for path_ids in raw_paths:
        ...
        asset_types      = [a.asset_type for a in path_assets]
        asset_names      = [a.name for a in path_assets]
        risk             = score_path(path_ids, assets)
        severity         = get_severity(risk)
        stride_threats   = get_stride_threats(asset_types)
        mitre_techniques = get_mitre_techniques(asset_types)

        attack_path = AttackPath(
            scenario_id      = scenario_id,
            path_sequence    = asset_names,
            risk_score       = risk,
            severity         = severity,
        ...
        db.add(attack_path)
        saved_paths.append(attack_path)

    db.add(AuditLog(user_id=USER_ID, scenario_id=scenario_id, action="analyzed"))
    db.commit()
