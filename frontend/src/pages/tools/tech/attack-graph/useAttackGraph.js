...
export function useAttackGraph() {
  ...
  const fetchScenarios = useCallback(async () => {
    try {
      const l = await api.getScenarios();
      setScenarios(Array.isArray(l) ? l : []);
  ...
  const createScenario = useCallback(async (name) => {
    ...
    try {
      ...
      // Validate the response has an id before using it
      if (!s?.id) throw new Error("Server returned a scenario with no id.");
      setScenario(s);
      setAssets([]);
      setConnections([]);
      setAnalysis(null);
  ...
  const loadScenario = useCallback(async (s) => {
    // Guard: if the scenario object has no id, bail out early
    if (!s?.id) {
      setError("Cannot load scenario: missing id. Try refreshing the list.");
      return;
    }
    setLoading(true);
    try {
  ...
  const addAsset = useCallback(async (sid, data) => {
    setLoading(true);
    try {
      const a = await api.addAsset(sid, data);
      if (a?.id) setAssets(prev => [...prev, a]);
      return a;
    } catch (e) { setError(e.message); }
    finally     { setLoading(false); }
  }, []);
  ...
  const removeAsset = useCallback(async (aid) => {
    try {
      ...
      setConnections(prev => prev.filter(
        c => String(c.source_id) !== String(aid) && String(c.target_id) !== String(aid)
      ));
    } catch (e) { setError(e.message); }
  }, []);
  ...
  const addConnection = useCallback(async (sid, data) => {
    setLoading(true);
    try {
      const c = await api.addConnection(sid, data);
      if (c?.id) setConnections(prev => [...prev, c]);
      return c;
    } catch (e) { setError(e.message); }
    finally     { setLoading(false); }
  }, []);
